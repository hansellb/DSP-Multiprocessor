 ...
 */
	while( TRUE )
	{
...
#if PERFORMANCE
				while( altera_avalon_mutex_trylock( mutex_0, ALT_CPU_CPU_ID_VALUE ) ){}
					put_img(img2, IMG_SIZE_16x16x1, IMG_4_16x16x1_ADDR);
				altera_avalon_mutex_unlock( mutex_0 );
#else
					put_img(img1+IMG_SIZE_20x20x1+8, ((*((uBYTE *)(img1+IMG_SIZE_20x20x1+8))) * (*((uBYTE *)(img1+IMG_SIZE_20x20x1+8 + 1)))), IMG_4_20x20x1_ADDR);
#endif
				...
				while( altera_avalon_mutex_trylock( mutex_1, ALT_CPU_CPU_ID_VALUE ) ){}
					...
					altera_avalon_fifo_write_fifo( FIFO_0_IN_BASE, FIFO_0_IN_CSR_BASE, IMG_4_16x16x1_ADDR );
				altera_avalon_mutex_unlock( mutex_1 );
#else
					altera_avalon_fifo_write_fifo( FIFO_0_IN_BASE, FIFO_0_IN_CSR_BASE, IMG_DSP_DONE_MSG );
					altera_avalon_fifo_write_fifo( FIFO_0_IN_BASE, FIFO_0_IN_CSR_BASE, IMG_4_20x20x1_ADDR );
#endif
